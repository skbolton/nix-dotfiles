...
vim.keymap.set('n', '<localleader>ft', function(args) get_todos(vim.fn.expand('%'), 'open') end, { buffer = true })
...
local clock_in = function()
  ...
  vim.fn.append(line, {
    "@code ledger",
    "i " .. os.date("%Y/%m/%d") .. " " .. os.date("%H:%M:00"),
    "@end"
  })
end
...
local clock_out = function()
  local line = vim.api.nvim_win_get_cursor(0)[1]
  vim.fn.append(line, {
    "o " .. os.date("%Y/%m/%d") .. " " .. os.date("%H:%M:00"),
  })
end
