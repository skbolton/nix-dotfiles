...
let
  ...
  findTreesitterGrammar =
    name:
    builtins.head (builtins.filter (grammar: lib.getName grammar == name) nvimTreesitterAllGrammars);
  ...
  overrideMarkdownGrammarExtensions =
    grammar:
  ...
  overriddenMarkdownGrammars = {
    tree-sitter-markdown = overrideMarkdownGrammarExtensions (
      findTreesitterGrammar "tree-sitter-markdown"
    );
    tree-sitter-markdown_inline = overrideMarkdownGrammarExtensions (
      findTreesitterGrammar "tree-sitter-markdown_inline"
    );
  };
  ...
  nvimTreesitterWithAllGrammarsAndMarkdownExtensions = pkgs.vimPlugins.nvim-treesitter.withPlugins (
    _:
    map (
      grammar:
      let
