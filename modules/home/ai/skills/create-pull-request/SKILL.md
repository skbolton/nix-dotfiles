...
1. [path/to/first-file](<link-to-diff>) — <one-line reason why to read this first>
2. [path/to/second-file](<link-to-diff>) — <one-line reason>
3. [path/to/third-file](<link-to-diff>) — <one-line reason>
...
```
...
1. [`lib/payments/retry_service.ex`](#diff-abc123) — new module; defines the interface everything else delegates to
2. [`lib/payments/retry_policy.ex`](#diff-def456) — retry policy rules consumed by the service
3. [`lib/payments/payments.ex`](#diff-789abc) — context module wiring that exposes the new service
4. [`lib/web/controllers/charge_controller.ex`](#diff-aaa111) — controller now delegates to service
5. [`lib/web/controllers/subscription_controller.ex`](#diff-bbb222) — same delegation pattern
6. [`lib/web/controllers/invoice_controller.ex`](#diff-ccc333) — same delegation pattern
7. [`test/payments/retry_service_test.exs`](#diff-ddd444) — verifies the new service behavior
```
