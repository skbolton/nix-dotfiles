...
GitHub's PR "Files changed" tab generates anchors for each file using the SHA-256 hash of the file path. Use these as relative links in the PR description so reviewers can click to jump directly to a file's diff.

**Format:**

```markdown
[`path/to/file.ex`](#diff-<sha256-of-path>)
```

**Computing the hash:**

```bash
echo -n "path/to/file.ex" | sha256sum | awk '{print $1}'
```
