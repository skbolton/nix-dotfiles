...
GitHub's PR "Files changed" tab generates anchors for each file using the SHA-256 hash of the file path. The links must point to the `/changes` sub-path of the PR, not just the PR root.

**Format:**

```markdown
[`path/to/file.ex`](changes#diff-<sha256-of-path>)
```

The link uses a relative path (`changes#diff-...`) so it resolves correctly from the PR page regardless of the repository URL.
...
git diff --name-only $(git merge-base HEAD <base-branch>)..HEAD | while read -r f; do
  ...
  echo "[$f](changes#diff-$hash)"
done
```

The anchor uses the full 64-character hex hash. GitHub will resolve it to the correct file in the diff view.
